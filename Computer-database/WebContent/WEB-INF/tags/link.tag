...
<c:choose>
	<c:when test="${active == true && btn == true}">
		<a class="btn btn-default" href="${target}?page=${page}&limit=${limit}">${body}</a>
	</c:when>
	<c:when test="${active == false && btn == true}">
		<a class="btn btn-default disabled" href="${target}?page=${page}&limit=${limit}">${body}</a>
	</c:when>
	<c:when test="${active == true && btn == false}">
		<a href="${target}?page=${page}&limit=${limit}">${body}</a>
	</c:when>
	<c:otherwise>
		<a class="disabled" href="${target}?page=${page}&limit=${limit}">${body}</a>
	</c:otherwise>
</c:choose>